...
nRaizes a b c | b^2 -4*a*c == 0 = 1
              | b^2 -4*a*c > 0 = 2                             | otherwise = 0 

raizes :: Float -> Float -> Float -> [Float]
raizes a b c | nRaizes a b c == 0 = []                                      | nRaizes a b c == 1 = [-b/2*a]                                | nRaizes a b c == 2 = [(-b + sqrt((b)^2 -4*a*c))/2*a,(-b - sqrt((b)^2 -4*a*c))/2*a]
...
comphoras (h1,m1) (h2,m2) | h1 > h2 = (h1,m1)
                              | h1 < h2 = (h2,m2)              | m1 > m2 = (h1,m1)              | otherwise = (h2,m2)
...
adicmin minx (hoy,miny) | minx + miny > 60 = (hoy + 1, minx + miny - 60)                                      | minx + miny < 60 = (hoy, minx + miny)
...
comphoras2 hora1@(H h1 m1) hora2@(H h2 m2)| h1 > h2 = hora1
                              | h1 < h2 = hora2              | m1 > m2 = hora1              | otherwise = hora2
...
adicmin2 minx (H hoy miny) | minx + miny > 60 = (H (hoy + 1) (minx + miny - 60))                                      | minx + miny < 60 = (H hoy (minx + miny))
